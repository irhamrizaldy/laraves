<template>
  <div>
    <div class="p-2 bg-gray-600 shadow-sm">
      <navbar class="justify-content-start nav-link">
    ...
    <div class="grid sm:grid-cols-12 md:grid-cols-4 gap-4 p-4">
      ...
      <div class="bg-white p-3 shadow-sm rounded">
        ...
        <div class="mt-4 pb-2">
          ...
          <select
            class="w-full border bg-white rounded px-3 py-2 outline-none"
            v-model="state.city_destination"
          >
            <option
              class="py-1"
              v-for="city in cities_destination"
              :key="city.id"
              :value="city.city_id"
            >
              {{ city.city_name }}
      ...
      <div class="bg-white p-3 shadow-sm rounded">
        ...
        <div class="mt-4 pb-2">
          ...
          <input
            type="text"
            class="p-2 bg-gray-200 rounded w-full shadow hover:bg-white"
    ...
    <div class="grid sm:grid-cols-1 md:grid-cols-1 gap-4 p-4" v-if="resultCost">
      <div class="bg-white p-3 shadow-sm rounded">
        ...
        <div class="mt-4" v-for="(value, index) in resultCost" :key="index">
          {{ value.service }} - {{ value.cost[0].value }} -
...
export default {
  ...
  setup() {
    ...
    function getCitiesOrigin() {
      axios
        .get(`http://localhost:8000/api/cities/${state.province_origin}`)
        .then((response) => {
    ...
    function getCitiesDestination() {
      axios
        .get(`http://localhost:8000/api/cities/${state.province_destination}`)
        .then((response) => {
    ...
    function getCostOngkir() {
      axios
        .post("http://localhost:8000/api/checkOngkir/", {
